...
class BookingController extends Controller
{
    ...
    public function uploadFile($id) //uploadFile ... 
    {

         $uploadFile = Booking::findOrFail($id);
    
        return view('bookings.upload-file')->with('uploadFile', $uploadFile);

    }

    public function StoreReservationFile(UpdateUploadFileRequest $request) //uploadFile ... 
    {


        try {

                $booking = $this->bookingRepository->findWithoutFail($request->id);
                $checkBookingStatus = $booking->bookingStatus->status;
                $input = $request->validated();

                if ($checkBookingStatus == "Done"){
                        
                    if($request->hasFile('upload_file'))
                    {

                         $imageName = time() . '.' . request()->upload_file->getClientOriginalExtension();
                         request()->upload_file->storeAs('public/bookings/',$imageName);
                         $input['upload_file'] =  asset('storage/bookings/'. $imageName);

                         $booking->update($input);
    
                        Flash::success(__('lang.upload_file_success', ['operator' => __('lang.booking')]));
                        return redirect(route('bookings.index'));
                ...
                Flash::warning(__('lang.upload_file_error'));
                return redirect(route('bookings.index'));
