...
class BookingAPIController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $input    =  $this->validate($request, Address::$rules+['e_service' => 'required|exists:e_services,id',
                ]);
            $data = $input ;  unset($data["e_service"]) ;
            $address = Auth::user()->addresses()->updateOrCreate($data);
            if (empty($address)) {
